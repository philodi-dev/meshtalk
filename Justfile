...
backup:
    ...
    @if [ -f MeshTalk.xcodeproj/project.pbxproj ]; then cp MeshTalk.xcodeproj/project.pbxproj MeshTalk.xcodeproj/project.pbxproj.backup; fi
    @if [ -f MeshTalk/Info.plist ]; then cp MeshTalk/Info.plist MeshTalk/Info.plist.backup; fi
...
restore:
    ...
    @if [ -f MeshTalk/LaunchScreen.storyboard.ios ]; then mv MeshTalk/LaunchScreen.storyboard.ios MeshTalk/LaunchScreen.storyboard; fi
    @# Use git to restore all modified files except Justfile
    @git checkout -- project.yml MeshTalk.xcodeproj/project.pbxproj MeshTalk/Info.plist 2>/dev/null || echo "⚠️  Could not restore some files with git"
    @# Remove any backup files
    @rm -f MeshTalk.xcodeproj/project.pbxproj.backup MeshTalk/Info.plist.backup 2>/dev/null || true
...
patch-for-macos: backup
    ...
    @if [ -f MeshTalk/LaunchScreen.storyboard ]; then mv MeshTalk/LaunchScreen.storyboard MeshTalk/LaunchScreen.storyboard.ios; fi
...
build: check generate
    @echo "Building MeshTalk for macOS..."
    @xcodebuild -project MeshTalk.xcodeproj -scheme "MeshTalk (macOS)" -configuration Debug CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGN_ENTITLEMENTS="" build

# Run the macOS app
run: build
    @echo "Launching MeshTalk..."
    @find ~/Library/Developer/Xcode/DerivedData -name "MeshTalk.app" -path "*/Debug/*" | head -1 | xargs -I {} open "{}"

# Clean build artifacts and restore original files
clean: restore
    @echo "Cleaning build artifacts..."
    @rm -rf ~/Library/Developer/Xcode/DerivedData/MeshTalk-* 2>/dev/null || true
    @# Only remove the generated project if we have a backup, otherwise use git
    @if [ -f MeshTalk.xcodeproj/project.pbxproj.backup ]; then \
        rm -rf MeshTalk.xcodeproj; \
    else \
        git checkout -- MeshTalk.xcodeproj/project.pbxproj 2>/dev/null || echo "⚠️  Could not restore project.pbxproj"; \
    fi
...
# Show app info
info:
    @echo "MeshTalk - Decentralized Mesh Messaging"
    @echo "======================================"
...
nuke:
    ...
    @rm -rf ~/Library/Developer/Xcode/DerivedData/MeshTalk-* 2>/dev/null || true
    @rm -rf MeshTalk.xcodeproj 2>/dev/null || true
    @rm -f project.yml.backup 2>/dev/null || true
    @rm -f project-macos.yml 2>/dev/null || true
    @rm -f MeshTalk.xcodeproj/project.pbxproj.backup 2>/dev/null || true
    @rm -f MeshTalk/Info.plist.backup 2>/dev/null || true
    @# Restore iOS-specific files if they were moved
    @if [ -f MeshTalk/LaunchScreen.storyboard.ios ]; then mv MeshTalk/LaunchScreen.storyboard.ios MeshTalk/LaunchScreen.storyboard; fi
    @git checkout -- project.yml MeshTalk.xcodeproj/project.pbxproj MeshTalk/Info.plist 2>/dev/null || echo "⚠️  Not a git repo or no changes to restore"
